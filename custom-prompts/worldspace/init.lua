...
local Icons = require(ReplicatedStorage.databases.icons)
...
local function createPrompt(prompt, inputType, gui)
	local promptActionType = prompt:GetAttribute('Type')
	...
	if inputType == Enum.ProximityPromptInputType.Gamepad then
		if Icons.GamepadButtonImage[prompt.GamepadKeyCode] then
			setupIconTweens()
			icon.Image = Icons.IconGamepadButtonImage[prompt.GamepadKeyCode]
	...
	elseif inputType == Enum.ProximityPromptInputType.Touch then
		...
		local buttonTextImage = Icons.KeyboardButtonImage[prompt.KeyboardKeyCode]
		if buttonTextImage == nil then
			buttonTextImage = Icons.KeyboardButtonIconMapping[buttonTextString]
		end

		if buttonTextImage == nil then
			local keyCodeMappedText = Icons.KeyCodeToTextMapping[prompt.KeyboardKeyCode]
	...
	triggeredConnection = prompt.Triggered:Connect(function()
		print(`Triggered in worldspace`)
		executor:execute(promptActionType, 'Triggered')
...
function worldspace:__init__()
	
end

function worldspace:__start__()
